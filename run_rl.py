...
def train_and_test(data_pkl="data_lagged.pkl", price_pkl="close_prices.pkl", test_year=None):
    ...
    if test_year is None:
        for year in dataset.keys():
            do_single_year(dataset, prices, year)
    else:
...
def do_single_year(dataset, prices, test_year):
    ...
    model.save("DQN-" + str(test_year) + ".sb3model")
